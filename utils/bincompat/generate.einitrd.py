...
initrd.cpio:
	cd rootfs && find -depth -print | tac | bsdcpio -o --format newc > ../initrd.cpio
"""
...
old="$PWD"
cd "$rootfs"
find -depth -print | tac | bsdcpio -o --format newc > "$old"/initrd.cpio
cd "$old"
"""
...
def generate_build_make(config, defconfig, plat, arch, comp):
    ...
    if comp == "gcc":
        compiler_var = "COMPILER=gcc"
    if comp == "clang":
        compiler_var = "CC=clang"
    contents = TEMPLATE_BUILD_MAKE.format(defconfig,
                                          compiler_var, config["name"], plat, arch,
                                          config["kerneldir"], comp, config["name"], plat, arch)
    base = os.path.basename(defconfig)
    out_file = os.path.join(config["builddir"], f"make-{comp}-{base}.sh")
...
def generate_build(config):
    ...
    for file in files(config["defconfigdir"]):
        ...
        generate_build_make(config, file, plat, arch, "gcc")
        generate_build_make(config, file, plat, arch, "clang")

    for target in config["targets"]:
        generate_build_kraft(config, target['plat'], target['arch'])


def generate_run_fc_json(config, plat, arch, filesystem):
    """Generate running config (JSON) for Firecracker."""

    kernel = os.path.join(config['kerneldir'], f"{config['name']}_{plat}-{arch}")

    json_name = os.path.join(config["rundir"], f"{plat}-{arch}-{filesystem}.json")
...
def generate_run_fc(config, plat, arch, filesystem):
    """Generate running script using Firecracker."""

    header = TEMPLATE_RUN_FIRECRACKER_HEADER.format(
            os.path.join(f"{config['rundir']}", f"{plat}-{arch}-{filesystem}.json")
            )

    out_file = os.path.join(config["rundir"], f"{plat}-{arch}-{filesystem}.sh")
    with open(out_file, "w", encoding="utf8") as stream:
        stream.write(header)
...
def generate_run_qemu(config, plat, arch, filesystem):
...
def generate_run(config):
    ...
    for target in config["targets"]:
        if target['plat'] == "fc" or \
            ...
            if config["rootfs"]:
                generate_run_fc_json(config, target['plat'], target['arch'], "initrd")
                generate_run_fc(config, target['plat'], target['arch'], "initrd")
            else:
                generate_run_fc_json(config, target['plat'], target['arch'], "nofs")
                generate_run_fc(config, target['plat'], target['arch'], "nofs")
        elif target['plat'] == "qemu":
            if config["rootfs"]:
                generate_run_qemu(config, target['plat'], target['arch'], "initrd")
                generate_run_qemu(config, target['plat'], target['arch'], "9pfs")
            else:
                generate_run_qemu(config, target['plat'], target['arch'], "nofs")
